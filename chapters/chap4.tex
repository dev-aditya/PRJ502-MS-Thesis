...
\refeq{eq:class_jcm_eq3} into \refeq{eq:class_jcm_eq_TISE} and obtain a set of coupled equations
...
We split the action of the total Hamiltonian on the field and quantum system such that the 
...
the velocity ${\dot{Q}}(t)$ is non-zero. But it's evident that the Time-Dependent  Schrödinger Equation (TDSE) of (\refeq{eq:class_jcm_eq17}) remains valid for all times. 
...
The above equation corresponds to \refeq{eq:class_jcm_eqA} of the position space approach. 
...
This ensures that the second part of \refeq{eq:class_jcm_eq23} disappears and one notices that
\begin{eqnarray}
